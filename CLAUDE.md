...
- **Une tab peut être jouée dans l'ampli, mais rendue avant d'être entendue, jamais en direct** (`engine/tab-*.ts`, `engine/di-*.ts`). Chaque piste de guitare de la partition est jouée par une banque de vraies notes DI puis passée dans `chain.wasm` — une instance à elle, dans un worker, comme l'export d'une prise — et tout le reste de la partition est rendu par le synthé d'alphaTab. Ce qui joue ensuite est un tampon par piste, démarré au même instant sur une seule horloge : c'est ce qui tient une grosse caisse et un chug sur la même milliseconde, ce que deux horloges ne feraient pas. alphaTab passe en mode média externe et son curseur suit ce rendu. Raison du rendu hors ligne : une piste amplifiée coûte à peu près ce que coûte la guitare du joueur (28 à 34 % d'un cœur sur un portable de 2017), donc trois pistes en direct, c'est le budget CPU explosé et des craquements garantis ; hors ligne, ça ne coûte rien au thread audio et un retard de calcul ne fait qu'attendre. Corollaire assumé : la vitesse de lecture ne bouge pas tant que l'ampli joue la tab — ralentir un rendu, c'est le refaire, et le rééchantillonner descendrait tout le morceau d'un ton. La banque DI est fabriquée par `scripts/build-di-bank.ts` à partir d'enregistrements ; elle n'est pas dans le dépôt, et rien ne la télécharge tant que personne n'a demandé l'ampli.
