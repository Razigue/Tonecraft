...
règle disait `1`, au motif que la chaîne est mono. La chaîne *est* mono jusqu'au
doubleur, son dernier étage — mais la **capture** ne doit pas l'être : une interface à deux entrées
...
- **Le doubleur est le seul étage stéréo, et il ne retarde rien** (`dsp/doubler.h`). Il simule une deuxième prise panoramiquée : oreille gauche = la chaîne, oreille droite = la même chaîne retardée d'un décalage qui erre au hasard entre 3 ms et le Spread (20 ms au plus) — la plage où l'effet de précédence fusionne encore les deux copies en une seule guitare large au lieu d'un écho, et celle du doubler d'Archetype: Tim Henson X. Il erre parce qu'une copie fixe est un filtre en peigne qui sonne comme une pièce ; sa pente est bornée à 0,002 (3,5 cents au pire) pour que l'errance ne s'entende pas comme un chorus. **Latence : nulle.** L'oreille gauche n'est pas retardée et la précédence localise le son à la première arrivée ; rien n'est donc ajouté au chiffre affiché. **Bypassé par défaut**, et gratuit tant qu'il l'est : l'oreille droite est alors une copie de la gauche au bit près (`npm run test:chain`). Actif, il coûte ~9 µs par bloc de 128 (0,3 % d'un cœur). Il prend la chaîne et la lecture du looper — une boucle, enregistrée mono, est doublée comme elle est entendue maintenant — mais ni le chemin direct de l'A/B ni la piste de fond. Deux limiteurs, un par oreille : ce sont des waveshapers, il n'y a rien à lier. Un export traité est stéréo si et seulement si le doubleur y est actif. La sortie droite est un export nouveau (`tc_output_right_ptr`) et non un changement de `tc_output_ptr` : un Tonecraft Engine plus ancien continue de fonctionner, en jouant l'oreille gauche des deux côtés.
...
- **La timeline est un éditeur, pas un dessin** (`cutTimeline` et les offsets de piste dans `engine/recording.ts`). Deux gestes, et ils vivent dans le modèle, pas dans le composant : **déplacer** une piste la décale dans le temps par sa poignée (jamais avant sa position alignée, donc rien ne tombe hors du début de la timeline et n'est perdu), et **couper** retire la sélection **de toutes les pistes à la fois**, en refermant le trou. De toutes, parce que la sélection est un segment de la timeline — c'est déjà ce à quoi l'export se coupe — et la retirer d'une seule décalerait cette piste contre les autres d'exactement ce qui a été enlevé : une prise et le morceau sur lequel elle a été jouée cesseraient de coïncider, ce que la timeline existe précisément pour tenir. Les deux gestes sont annulables une fois, écrits sur la prise stockée, et respectés par l'export comme par l'écoute.
- **La piste de fond joue dans la chaîne, jamais dans une prise** (`tc_backing_*`, `dsp/chain.cpp`). Elle rejoint le mix après le volume de sortie, qui ne règle que la guitare, et avant le limiteur, qui protège toujours le casque : elle a son propre niveau, le looper ne l'enregistre pas, le DI ne la contient pas. Armée, elle démarre sur le bloc où démarre l'enregistrement et s'arrête avec lui, donc prise et piste sont alignées à l'échantillon dans la chaîne. À l'export, la guitare est avancée de la latence aller-retour mesurée à l'arrêt de la prise, pour sonner comme elle a été jouée sur ce qui était entendu ; l'export coupe la timeline à la sélection, sinon du début à la fin de la plus longue des pistes exportées. Coût : une lecture et une addition par échantillon quand elle joue, latence nulle.
- **Passer sur un fichier ne rend pas l'interface.** Le changement de source ferme la capture à la source (`setCaptureOpen`, instantané) ; il ne relâche jamais le périphérique, qui n'est rendu qu'à l'arrêt du moteur. Rouvrir une interface coûte quelques centaines de millisecondes — ou échoue, si un autre logiciel l'a prise entre-temps —, et c'est ce qui éteignait la tête d'ampli à la fin de chaque prise seulement écoutée : la prise s'arrêtait, le micro n'était pas revenu, et le vitrail retombait à son plancher avec la guitare muette derrière. Vérifié par `npm run test:studio` : écouter une prise n'appelle pas `getUserMedia`.
...
- **La page propose la mise à jour du service, explicitement** (`engine/engine-update.ts`). Quand le rig démarre sur le moteur natif, elle compare la version annoncée au `hello` à la dernière release `engine-v*` publiée — API publique GitHub, CORS ouvert, gratuite, réponse gardée 6 h pour rester sous 60 requêtes par heure — et, s'il est plus ancien, affiche un bouton « Update Tonecraft Engine » qui télécharge la release. Un échec de la requête ne dit rien. Le service ne se met pas à jour lui-même : les versions déjà installées n'en ont pas le code.
- **Chez le joueur, le service n'est qu'une icône.** Il démarre minimisé, sans fenêtre ; un clic ouvre Tonecraft dans le navigateur, le menu propose « Open Tonecraft », « Start with this computer » et « Quit ». Tout se règle dans Tonecraft : interface, buffer, fréquence, canaux et volume casque. Le joueur doit avoir l'impression de ne se servir que du navigateur. Corollaire : **fermer l'onglet coupe le son** — le service arrête les flux et libère le pilote dès que la page se déconnecte. Le programme, lui, ne se ferme pas avec l'onglet : il reste près de l'horloge, inactif, jusqu'à « Quit » — les joueurs l'ont préféré ainsi.
...
- **Latence minimale côté service** : buffer par défaut = le plus petit accepté par le périphérique, **sauf en ASIO**, où buffer et fréquence restent ceux du panneau de l'interface tant que le joueur n'en choisit pas d'autres dans Tonecraft — un buffer ASIO appartient au pilote et à tous les logiciels qui s'en servent, le service ne l'écrase jamais d'office ; en ASIO, le flux d'entrée est enregistré avant la sortie, donc le bloc capturé est traité et rejoué dans le même `bufferSwitch` ; l'anneau entre les deux callbacks ne garde jamais de retard (0 bloc en ASIO, 1 ailleurs), le surplus est jeté et compté comme dropout.
...
- **Le studio ne scrolle pas, côté musicien.** Une seule fenêtre fixe, `100dvh`, quatre bandes pleine largeur : barre (56 px), chaîne (80 px), scène élastique, transport. Le transport est le DAW du studio : sa rangée (88 px) et, dessous, les pistes du recorder, toujours visibles ; la scène cède la hauteur qu'elles prennent. **La tête d'ampli a une seule taille et une seule forme** : elle ne se réorganise jamais et son vitrail ne rétrécit pas — le studio la met à l'échelle entière (`zoom`) pour tenir dans la place qui reste, en largeur comme en hauteur, comme s'éloigner d'un ampli le rend plus petit et non plus plat. Son rapport largeur/hauteur est donc constant à toutes les tailles d'écran. Ce qui s'ouvre encore (pistes de la tab, niveaux) se superpose à la scène. Le seul contenu qui défile est la tab. Vérifié par `npm run test:studio` (`scrollHeight === innerHeight` dans les deux vues). La page du testeur, elle, défile : voir la règle suivante.
...
- **Le lecteur de tablatures ne touche pas à la chaîne en temps réel, et la chaîne ne l'attend pas.** Rendu en SVG, jamais en canvas — alphaTab sait faire les deux, le choix est écrit dans la config plutôt que hérité d'un défaut. Sa lecture audio a son propre `AudioContext` : elle ne partage ni le graphe, ni l'horloge, ni le worklet, et n'entre donc ni dans le looper ni dans une prise. Le moteur, les polices de notation et le soundfont sont chargés au premier fichier ouvert, pas avant.
- **Une tab peut être jouée dans l'ampli, mais rendue avant d'être entendue, jamais en direct** (`engine/tab-*.ts`, `engine/di-*.ts`). Chaque piste de guitare de la partition est jouée par une banque de vraies notes DI puis passée dans `chain.wasm` — une instance par worker, comme l'export d'une prise — et tout le reste de la partition est rendu par le synthé d'alphaTab. Ce qui joue ensuite est une suite de tampons par piste, posés sur une seule horloge : c'est ce qui tient une grosse caisse et un chug sur la même milliseconde, ce que deux horloges ne feraient pas. alphaTab passe en mode média externe et son curseur suit ce rendu. Raison du rendu hors ligne : une piste amplifiée coûte à peu près ce que coûte la guitare du joueur (28 à 34 % d'un cœur sur un portable de 2017), donc trois pistes en direct, c'est le budget CPU explosé et des craquements garantis ; hors ligne, ça ne coûte rien au thread audio et un retard de calcul ne fait qu'attendre. **La lecture démarre avant la fin du rendu** : les pistes se rendent par tranches, dans l'ordre où on les entendra, et le départ est décidé par la vitesse mesurée du rendu — avec `v` secondes de musique par seconde, démarrer avec `r` rendu est sûr quand `r >= (1 - v) × durée`. Mesuré : Archspire, sept pistes, 4 min 17, se rend à mi-vitesse sur deux cœurs — jouable à 4,3 min, fini à 8,2 ; deux pistes d'une tab de 80 s sont jouables au cinquième du rendu. Ralentir une tab, c'est la **rendre à nouveau** à cette vitesse — les événements des guitares sont étirés, le groupe est exporté depuis une partition dont les tempos sont mis à l'échelle le temps de générer son MIDI ; rééchantillonner descendrait tout le morceau d'un ton. La lecture reprend où elle en était dès que le rendu y est revenu, et la synchro au métronome est la même chose avec le rapport que demande le clic. Corollaire assumé : le niveau d'entrée de l'ampli est estimé depuis les notes (à ~1 dB près) plutôt que mesuré sur une piste qui n'existe pas encore. La banque DI est fabriquée par `scripts/build-di-bank.ts` à partir d'**EG-IPT** (Zenodo 15205644, **CC BY-4.0** : dérivés et usage commercial autorisés contre attribution, portée par le pied de page et `assets/README.md`) — une Gibson SG en DI, 96 kHz, une note par fichier, dont on tire les 138 notes piquées, les 138 palm mutes et les harmoniques du micro chevalet. Le choix du jeu de données est d'abord un choix de licence : une banque taillée dans un jeu **ND** ne serait jamais publiable, quoi qu'elle sonne. Elle est commitée comme la soundfont (47 Mo, AD-15 : le déploiement reste une pure fonction du commit) et rien ne la télécharge tant que personne n'a demandé l'ampli.
