...
| Tab reader soundfont | **Third-party, unmodified.** Served as `public/musescore-general/MuseScore_General.sf3`: MuseScore_General v0.2.0, MIT, 39 900 972 bytes, SHA-256 `5b85b6c2c61d10b2b91cddd41efcce7b25cd31c8271d511c73afafbef20b6fa3`, from `ftp.osuosl.org/pub/musescore/soundfont/MuseScore_General/`. Committed rather than fetched in CI so the deploy stays a pure function of the commit; the cost is 40 MB in history for good. It is fetched only when a tab is opened. It is voiced for FluidSynth, and alphaTab differs where the pianos need it: stereo samples it skipped, global zone attenuation it counts twice, and the modulators that open the piano's filter, which it ignores. The reader corrects those in memory before loading (`engine/soundfont.ts`), so the file on disk stays exactly this one. |
