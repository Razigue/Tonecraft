...
self.onmessage = async (event: MessageEvent<{ takes: Recording[]; sampleRate: number; tone: RecordingTone | null; base: string; options?: ExportOptions }>) => {
  try {
    ...
    const renderedRight: (Float32Array | undefined)[] = takes.map(() => undefined);
    ...
    if (tone && toRender.length > 0) {
      ...
      for (const [k, i] of toRender.entries()) {
        const take = await renderRecording(takes[i]!, tone, wasmBytes, modelBytes,
          progress => self.postMessage({ progress: (k + progress) / toRender.length }));
        rendered[i] = take.samples;
        renderedRight[i] = take.right;
      }
    }
    const mix = (ear: readonly Float32Array[]) => mixTimeline({
      guitars: takes.map((t, i) => ({ samples: ear[i]!, level: options.guitarLevels?.[i] ?? 1, latencyFrames: t.latencyFrames ?? 0,
    ...
    const samples = mix(rendered);
    if (samples.length === 0) throw new Error('There is nothing to export in this selection.');
    // A doubled track makes the file stereo; the other tracks sit in the middle.
    const right = renderedRight.some((r) => r !== undefined)
      ? mix(rendered.map((l, i) => renderedRight[i] ?? l)) : undefined;
    const wav = encodeWav(right === undefined ? { samples, sampleRate } : { samples, right, sampleRate });
