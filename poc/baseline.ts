...
import { loadScore, span, timeline } from './tab-events.ts';
...
export function renderAlphaTab(score: alpha.model.Score, keep: number[], barFrom: number, barTo: number, rate = 48000): [Float32Array, Float32Array] {
  ...
  // Played bars, so the same ticks as the guitars' timeline, repeats included.
  const bars = timeline(score).bars;
  range.startTick = bars[barFrom]!.start; range.endTick = bars[Math.min(bars.length - 1, barTo)]!.end;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(`wrote ${out}.wav ${(L.length / 48000).toFixed(1)} s (expected ${(span(score, barFrom, barTo)[1] - span(score, barFrom, barTo)[0]).toFixed(1)} s)`);
}
