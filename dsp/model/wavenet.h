...
//
// This replaces the hand-tuned Faust cascade that used to live in
// `dsp/amp/faust/`. The reason is measured rather than aesthetic. Against a
// commercial reference on the same DI, the cascade sat 19 dB down across 1.6 to
// 5 kHz and kept 2.4 dB of level between the softest and loudest attack of a
// run where the reference kept 14.3. Six soft clippers in series — the boost,
// four preamp stages and the power stage — multiply each other's compression,
// and five rounds of retuning moved that problem around without removing it.
// A trained network has one non-linearity and it learned the right one.
//
...
//
// **It runs at the base rate, outside the oversampling window, and that is a
...
// same reason. The window disappears with the Faust cascade, and its half-band
// filters take ~0.3 ms of round trip with them.
//
// **Cost, measured**, this implementation under the flags `dsp/build.sh` ships,
// in wasm SIMD128 on an i5-7300U — a 2017 dual-core laptop, the floor machine's
// class — at 128-frame blocks, as a share of one core:
//
//     Standard  16/8 channels   23.8 %      NAM's own, via Eigen:  22.6 %
//     Lite      12/6            19.9 %                             16.7 %
//     Feather    8/4             9.0 %                              9.6 %
//     Nano       4/2             5.3 %                              5.6 %
//
// `PRODUCT.md` §5 caps the whole chain at 25 % and the rest of it measures
// 5.1 % on that machine, so Standard does not fit and Feather does, with room
// left for the reverb. We beat Eigen at the narrow widths and lose to it at 16
// channels, which is the shape of the trade: Eigen blocks its GEMM for cache,
// and at eight channels there is no cache problem to solve, only overhead.
//
// Which size ships is decided once, at build time, for the floor machine, and
// never at runtime from measured headroom (AD-5): an adaptive engine would make
// a shared tone link sound different for the person who receives it.
